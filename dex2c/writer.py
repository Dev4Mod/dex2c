...
class Writer(object):
    ...
    def write_method(self):
        ...
        if self.irmethod.params_type:
            proto = ", ".join(
                [
                    "%s p%s" % (get_native_type(p_type), param)
    ...
    def visit_landing_pad(self, landing_pad):
        ...
        for atype, handle in landing_pad.handles.items():
            self.write(
                'if(d2c_is_instance_of(env, exception, "%s")) {\n' % (get_type(atype))
            )
            self.write("goto L%d;\n" % handle.num)
            self.write("}\n")
    ...
    def visit_load_constant(self, ins, obfus=False):
        ...
        if atype == "F":
            ...
            c = (
                'v%s = (%s) env->NewStringUTF(AY_OBFUSCATE("%s"));\n'
                if obfus
                else 'v%s = (%s) env->NewStringUTF("%s");\n'
            )
            self.write(c % (self.ra(val), get_native_type(atype), cst))
            ...
            self.write("v%s = (%s) %r;\n" % (self.ra(val), get_cdecl_type(atype), cst))
    ...
    def visit_move_result(self, ins, lhs, rhs):
        ...
        self.write(
            ...
            % (
                self.ra(lhs),
                get_cdecl_type(lhs.get_type()),
                self.get_variable_or_const(rhs),
            )
        )
    ...
    def visit_move(self, lhs, rhs):
        ...
        if is_primitive_type(rhs.get_type()):
            ...
            self.write(
                "(%s) env->NewLocalRef(v%s)"
                % (get_cdecl_type(lhs.get_type()), self.ra(rhs))
            )
        self.write(";\n")
    ...
    def visit_astore(self, ins, array, index, rhs):
        ...
        if is_primitive_type(elem_type):
            self.write(
                "{%s val = %s;"
                % (get_native_type(elem_type), self.get_variable_or_const(rhs))
            )
            self.write(
                "env->Set%sArrayRegion((%sArray) v%s, (jint) %s, 1, &val);"
                % (
            ...
            self.write(
                "env->SetObjectArrayElement((jobjectArray) v%s, (jint) %s, v%s);"
                % (self.ra(array), self.get_variable_or_const(index), self.ra(rhs))
            )
        self.write_undefine_ex_handle(ins)
        self.write("}\n")
    ...
    def visit_put_static(self, ins, clsdesc, name, ftype, rhs):
        ...
        self.write(
            'D2C_RESOLVE_STATIC_FIELD(clz, fld, "%s", "%s", "%s");\n'
            % (get_type(clsdesc), name, ftype)
        )
        self.write(
            "env->SetStatic%sField(clz,fld,(%s) %s);\n"
            % (
                get_type_descriptor(ftype),
                get_native_type(ftype),
                self.get_variable_or_const(rhs),
            )
        )
        self.write_undefine_ex_handle(ins)
    ...
    def visit_put_instance(self, ins, lhs, rhs, ftype, clsdesc, name):
        ...
        self.write("jfieldID &fld = %s;\n" % (self.fa(ins.get_field())))
        self.write(
            'D2C_RESOLVE_FIELD(clz, fld, "%s", "%s", "%s");\n'
            % (get_type(clsdesc), name, ftype)
        )
        self.write(
            "env->Set%sField(v%s,fld,(%s) %s);\n"
            % (
                get_type_descriptor(ftype),
                self.ra(lhs),
                get_native_type(ftype),
    ...
    def visit_new(self, ins, result, atype):
        ...
        self.write('D2C_RESOLVE_CLASS(clz,"%s");\n' % (get_type(atype)))
        self.write(
            "v%s = (%s) env->AllocObject(clz);\n"
            % (self.ra(result), get_native_type(result.get_type()))
        )
        self.write_undefine_ex_handle(ins)
        self.write("}\n")

    def get_jvalue_arg_expr(self, arg, atype):
        expr = self.get_variable_or_const(arg)
        sig = atype[0]

        if sig == "L" or sig == "[":
            return "(jobject) %s" % expr

        arg_type = arg.get_type()
        ref_to_primitive = util.is_ref(arg_type)

        if sig == "Z":
            return (
                "(jboolean) (uintptr_t) %s" % expr
                if ref_to_primitive
                else "(jboolean) %s" % expr
            )
        elif sig == "B":
            return (
                "(jbyte) (uintptr_t) %s" % expr
                if ref_to_primitive
                else "(jbyte) %s" % expr
            )
        elif sig == "C":
            return (
                "(jchar) (uintptr_t) %s" % expr
                if ref_to_primitive
                else "(jchar) %s" % expr
            )
        elif sig == "S":
            return (
                "(jshort) (uintptr_t) %s" % expr
                if ref_to_primitive
                else "(jshort) %s" % expr
            )
        elif sig == "I":
            return (
                "(jint) (uintptr_t) %s" % expr
                if ref_to_primitive
                else "(jint) %s" % expr
            )
        elif sig == "J":
            return (
                "(jlong) (uintptr_t) %s" % expr
                if ref_to_primitive
                else "(jlong) %s" % expr
            )
        elif sig == "F":
            return (
                "(jfloat) (uintptr_t) %s" % expr
                if ref_to_primitive
                else "(jfloat) %s" % expr
            )
        elif sig == "D":
            return (
                "(jdouble) (uintptr_t) %s" % expr
                if ref_to_primitive
                else "(jdouble) %s" % expr
            )
        else:
    ...
    def _invoke_common(self, ins, invoke_type, name, base, ptype, rtype, args, clsdesc):
        ...
        if invoke_type != "static":
            self.write(
                'D2C_RESOLVE_METHOD(clz, mid, "%s", "%s", "(%s)%s");\n'
                % (get_type(clsdesc), name, "".join(ptype), rtype)
            )
        else:
            self.write(
                'D2C_RESOLVE_STATIC_METHOD(clz, mid, "%s", "%s", "(%s)%s");\n'
                % (get_type(clsdesc), name, "".join(ptype), rtype)
            )
        self.write("jvalue args[] = {")
        vars = []
        for arg, atype in zip(args, ptype):
            expr = self.get_jvalue_arg_expr(arg, atype)
            if atype[0] == "L" or atype[0] == "[":
                vars.append("{.l = %s}" % expr)
            elif atype[0] == "Z":
                vars.append("{.z = %s}" % expr)
            elif atype[0] == "B":
                vars.append("{.b = %s}" % expr)
            elif atype[0] == "C":
                vars.append("{.c = %s}" % expr)
            elif atype[0] == "S":
                vars.append("{.s = %s}" % expr)
            elif atype[0] == "I":
                vars.append("{.i = %s}" % expr)
            elif atype[0] == "J":
                vars.append("{.j = %s}" % expr)
            elif atype[0] == "F":
                vars.append("{.f = %s}" % expr)
            elif atype[0] == "D":
                vars.append("{.d = %s}" % expr)
            else:
                raise Exception("Unknow signuare type %s" % (atype))
        ...
        if invoke_type == "super":
            ...
            self.write(
                "env->CallStatic%sMethodA(clz, mid, args);\n"
                % (get_type_descriptor(rtype))
            )
        else:
            self.write(
                "env->Call%sMethodA(v%s, mid, args);\n"
                % (get_type_descriptor(rtype), self.ra(base))
            )
    ...
    def visit_invoke(self, ins, invoke_type, name, base, ptype, rtype, args, clsdesc):
        self.write_trace(ins)
        return self._invoke_common(
            ins, invoke_type, name, base, ptype, rtype, args, clsdesc
        )
    ...
    def visit_instanceof(self, ins, result, arg, atype):
        ...
        self.write('D2C_RESOLVE_CLASS(clz,"%s");\n' % (get_type(atype)))
        self.write(
            "v%s = d2c_is_instance_of(env, v%s, clz);\n"
    ...
    def visit_aload(self, ins, result, array, index):
        ...
        if is_primitive_type(elem_type):
            ...
            self.write(
                "env->Get%sArrayRegion((%sArray) v%s, (jint) %s, 1, &val);"
                % (
                    get_type_descriptor(elem_type),
                    get_native_type(elem_type),
                    self.ra(array),
            ...
            self.write(
                "v%s = (%s) env->GetObjectArrayElement((jobjectArray) v%s, (jint) %s);"
                % (
                    self.ra(result),
                    get_native_type(result.get_type()),
                    self.ra(array),
                    self.get_variable_or_const(index),
                )
            )
    ...
    def visit_alength(self, ins, result, array):
        ...
        self.write_not_null(array)
        self.write(
            "v%s = env->GetArrayLength((jarray) v%s);\n"
    ...
    def visit_new_array(self, ins, result, atype, size):
        ...
        if is_primitive_type(elem_type):
            ...
            result.visit(self)
            self.write(
                " = env->NewObjectArray((jint) %s, clz, NULL);\n"
                % (self.get_variable_or_const(size))
            )
        self.write_undefine_ex_handle(ins)
        self.write("}\n")
    ...
    def visit_filled_new_array(self, ins, result, atype, size, args):
        ...
        self.write(
            'd2c_filled_new_array(env, (jarray) v%s, "%s", %d, %s);\n'
    ...
    def visit_monitor_enter(self, ins, ref):
        ...
        self.write("{\n")
        self.write_define_ex_handle(ins)
        ref_obj = "(jobject) %s" % (self.get_variable_or_const(ref))
        self.write("D2C_NOT_NULL(%s);\n" % (ref_obj))
    ...
    def visit_monitor_exit(self, ins, ref):
        ...
        self.write_define_ex_handle(ins)
        self.write(
            "if (env->MonitorExit((jobject) %s) != JNI_OK) {\n"
    ...
    def visit_get_static(self, ins, result, ftype, clsdesc, name):
        ...
        result.visit(self)
        self.write(
            " = (%s) env->GetStatic%sField(clz,fld);\n"
